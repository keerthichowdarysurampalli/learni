...
export const getColEditToggle = derived(
	[col1EditToggle, col2EditToggle, col3EditToggle],
	([$col1EditToggle, $col2EditToggle, $col3EditToggle]) => {
		return (col: number) => {
			if (col === 1) {
...
const _setCol = (layout: number, col: number, ...widgets: number[]) => {
	...
	switch (col) {
		...
		case 3:
			const rem3 = Math.floor(layout / 100) * 100;
			return widgets.length === 2 ? rem3 + (widgets[0] * 10 + widgets[1]) : rem3 + widgets[0] * 10;
		default:
			return layout;
	}
};
...
export const _getCol = (layout: number, col: number) => {
	switch (col) {
		case 1:
			return Math.floor(layout / 10000);
		case 2:
			return Math.floor(layout / 100) % 100;
		case 3:
			return layout % 100;
		default:
			return 0;
